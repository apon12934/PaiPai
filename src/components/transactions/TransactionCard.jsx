...
export default function TransactionCard({ transaction, onEdit, onDelete, isEditing }) {
    ...
    >
      ...
      <div className="mt-2 mb-1">
        <span className={`text-xl font-bold ${isGave ? 'text-rose-400' : 'text-emerald-400'}`}>
          {isGave ? '-' : '+'}৳{transaction.amount.toFixed(2)}
        </span>
      </div>

      {transaction.note && (
        <div className="mt-2 text-sm text-slate-400 bg-black/20 p-2 rounded-md">
          {transaction.note}
        </div>
      )}

      {/* Actions overlay */}
      <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-[#0B1120]/80 backdrop-blur-sm p-1 rounded-md border border-white/5">
        <button
          onClick={onEdit}
